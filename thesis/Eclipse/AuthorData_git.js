function mape1() {
 ...
 var color = d3.scale.threshold()
  ...
  var svg = d3.select("#agit1").append("svg")
                          .attr("width",width)
                          .attr("height",height)
  ...
  d3.json('thesis/Eclipse/data/scm-timezone.json', function(data){
 ...
 var tzdata = topojson.feature(json, json.objects.timezones).features;
    ...
    for(var i = 0;i < data.tz.length;i++){
      ...
      var authorValue = parseInt(data.authors[i]);
       ...
       var commitsByauthorsInv = parseFloat(data.authors[i]/data.commits[i]);
