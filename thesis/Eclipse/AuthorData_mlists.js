function mape2() {
  ...
  var ext_color_domain = [50, 250, 500, 1500, 2500]
  var legend_labels = ["< 50", "50 - 250", "250 - 500", "500 - 1500", "> 1500"]
 ...
 var color = d3.scale.threshold()
  ...
  var svg = d3.select("#aml1").append("svg")
                          .attr("width",width)
                          .attr("height",height)
  ...
  d3.json('thesis/Eclipse/data/mls-timezone.json', function(data){
   var com = data.authors;
   d.push(com);

  d3.json("thesis/Eclipse/data/timezones-topo2.json", function(json){
 ...
 var tzdata = topojson.feature(json, json.objects.timezones).features;
    ...
    for(var i = 0;i < data.tz.length;i++){
      ...
      var commitsByauthorsInv = parseFloat(data.authors[i]/data.messages[i]);
      //parseFloat(data.commitsByauthors[i]);
      ...
      for(var j = 0;j<tzdata.length;j++){
        ...
        if(dataTZ == jsonTZ) {
          ...
          tzdata[j].properties.div = commitsByauthorsInv;
